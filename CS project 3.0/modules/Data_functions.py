...
# This fuction verifies user from user table from HOTEL MANAGEMENT database
...
# This fuction changes the formating of date
...
# This fuction books a room in advance
...
# This fuction apply discounts in the bill
...
# This fuction updates payement details during booking
def pay_entry(booking_id):
    ...
    return {"roomCharges":roomCharge,"roomService":int(room_service_tax),"service":int(service_tax),"preference":prefCharge,"vat":int(vat),"gym":gym_pay,"bed":bed_pay,"breakfast":breakfast_pay,"bar":bar_pay,"discount":int(disc),"total":amount_to_be_paid}


# This fuction sorts the records collected from Mother table of the Hotel Management database
def sorting_time2():
    conn = sqlite3.connect("HOTEL MANAGEMENT.db")
    cursor = conn.cursor()
    ...
    for item in data:
        ...
        check_out_date = datetime.datetime.strptime(item[13], '%d-%m-%Y').date()
        reserved_dates = item[14].count(',') + 1
        if check_out_date < today:
            past.append((check_in_date, reserved_dates, item))
        elif check_in_date == today or check_out_date > today:
...
# This fuction updates the database for the bill being paid
def bill_paid(booking_id):
    ...
    cursor.execute(''' UPDATE Mother_table SET Amount_payable = ? WHERE booking_id = ? ''' ,
                (amount, booking_id))
    cursor.execute('''UPDATE payement_table SET status = 'done' WHERE booking_id = ?''', (booking_id,))
    conn.commit()
    conn.close()

# This fuction returns payement details from the payement table in the database
def pay_data():
    conn = sqlite3.connect("HOTEL MANAGEMENT.db")
    cursor = conn.cursor()
